%
%  Script calcualting Krawtchouk moments 
%  Implemented as discribed by https://doi.org/10.1007/s10851-017-0758-9
...
function KM = KM_all(img,p,order)
    ...
    for n= 0:order % change for other dimensions
        ...
        for s = 0:(b-1)
            for t = 0:(b-1)
                ...
                sum1 = sum1 + Rn(n+1,s+1)*Rm(m+1,t+1)*img(s+1, t+1);
            end
        end
        KM(n+1,m+1) = sum1;
...
function RMF = KM_basis(p,order,N)
for  x = 0:N-1
